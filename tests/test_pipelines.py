...
def test_extract_entities_empty(mock_llm):
    ...
    result = extract_entities("This note has no entities worth mentioning.", path="test_none.md")
...
def test_extract_entities_parses_json(mock_llm):
    ...
    result = extract_entities("Using ESP32 for IoT", path="test_hw.md")
    assert len(result) == 1
...
def test_extract_entities_multiple(mock_llm):
    ...
    result = extract_entities("Alice worked on ProjectX using ESP32", path="test_multi.md")
    assert len(result) == 3
...
def test_extract_entities_invalid_json_fallback(mock_llm):
    ...
    result = extract_entities("Using ESP32", path="test_fallback.md")
    assert len(result) == 1
    assert result[0]["name"] == "ESP32"
...
def test_extract_entities_broken_json(mock_llm):
    ...
    result = extract_entities("Some text", path="test_broken.md")
    assert result == []
...
def test_extract_entities_caching(mock_llm):
    ...
    result1 = extract_entities("Alice text", path="test_cache.md")
    assert len(result1) == 1
    # Second call with same path should use cache, not LLM
    mock_llm.chat.reset_mock()
    result2 = extract_entities("Alice text", path="test_cache.md")
...
def test_extract_entities_validates_types(mock_llm):
    ...
    result = extract_entities("test", path="test_validate.md")
