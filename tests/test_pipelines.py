...
def test_extract_entities_empty(mock_llm):
    ...
    mock_llm.chat.return_value = '{"entities": [], "summary": ""}'
    result, rels, summary = extract_entities("This note has no entities worth mentioning.", path="test_none.md")
...
def test_extract_entities_parses_json(mock_llm):
    _EXTRACT_ENTITIES_CACHE.clear()
    mock_llm.chat.return_value = '{"entities": [{"name": "ESP32", "type": "Hardware", "confidence": 0.95}], "summary": "Using ESP32 for IoT projects."}'
    result, rels, summary = extract_entities("Using ESP32 for IoT", path="test_hw.md")
    assert len(result) == 1
    assert result[0]["name"] == "ESP32"
    assert result[0]["type"] == "Hardware"
    assert result[0]["confidence"] == 0.95
    assert summary == "Using ESP32 for IoT projects."
...
def test_extract_entities_multiple(mock_llm):
    ...
    mock_llm.chat.return_value = (
        ...
        '], "summary": "Alice worked on ProjectX using ESP32."}'
    )
    result, rels, summary = extract_entities("Alice worked on ProjectX using ESP32", path="test_multi.md")
    assert len(result) == 3
    names = {e["name"] for e in result}
    assert names == {"Alice", "ProjectX", "ESP32"}
    assert "ProjectX" in summary
...
def test_extract_entities_invalid_json_fallback(mock_llm):
    ...
    mock_llm.chat.return_value = (
        ...
        '{"entities": [{"name": "ESP32", "type": "Hardware", "confidence": 0.95}], "summary": "Using ESP32."}\n'
        "That's all."
    )
    result, rels, summary = extract_entities("Using ESP32", path="test_fallback.md")
    assert len(result) == 1
    assert result[0]["name"] == "ESP32"
    assert "ESP32" in summary
...
def test_extract_entities_broken_json(mock_llm):
    ...
    result, rels, summary = extract_entities("Some text", path="test_broken.md")
    assert result == []
    assert summary == ""
...
def test_extract_entities_caching(mock_llm):
    _EXTRACT_ENTITIES_CACHE.clear()
    mock_llm.chat.return_value = '{"entities": [{"name": "Alice", "type": "Person", "confidence": 0.95}], "summary": "About Alice."}'
    result1, rels1, summary1 = extract_entities("Alice text", path="test_cache.md")
    assert len(result1) == 1
    assert summary1 == "About Alice."
    # Second call with same path should use cache, not LLM
    mock_llm.chat.reset_mock()
    result2, rels2, summary2 = extract_entities("Alice text", path="test_cache.md")
    assert len(result2) == 1
    assert summary2 == "About Alice."
...
def test_extract_entities_validates_types(mock_llm):
    ...
    result, rels, summary = extract_entities("test", path="test_validate.md")
